...
DGMS Coal Mine Safety Measurement Chart1. Ventilation & Gas Standards (Underground)In underground mines, gas levels and air quantity must be measured regularly to ensure a safe working environment.ParameterLimit / StandardRegulation ReferenceInflammable Gas (Methane)Shall not exceed 0.75% in the general body of air.CMR Reg 166/169Carbon Monoxide (CO)Must be kept below 50 ppm (parts per million).DGMS CircularAir VelocityMinimum 30 meters/minute (at the last ventilation connection).CMR Reg 153Oxygen LevelMinimum 19% required in the general body of air.CMR Reg 1532. Illumination (Lighting) StandardsDGMS has prescribed minimum illumination levels (Lux levels) for various areas within the mine.LocationMinimum Illumination (Lux)Haul Roads (Opencast)0.5 to 3.0 Lux (horizontal)Stock Pile Area15.0 Lux (horizontal)Crushing Plants / CHP20.0 to 50.0 LuxDrilling / Blasting Area10.0 Lux3. Dust & Noise ControlControlling dust and noise levels is a statutory requirement to protect the health of the workers.ParameterPermissible LimitMeasurement FrequencyRespirable Dust2 mg/m³ (if silica content is less than 5%)Every 30 days (minimum)Noise Level85 dB(A) for 8-hour shift (Warning level)Periodic SurveyVibration (HEMM)0.5 m/s² (Weighted RMS acceleration)Continuous Monitoring4. HEMM (Heavy Earth Moving Machinery) Safety FeaturesAccording to DGMS Tech Circular 06/2020, the following safety features are mandatory for Opencast mines:AVAS (Audio-Visual Alarm): A buzzer and light must activate when the reverse gear is engaged.Proximity Warning System: Alerts the driver if anyone or any object comes too close to the machine.Automatic Fire Suppression System (AFSS): Automatically extinguishes engine fires.Operator Fatigue Monitoring: A system to detect if the driver is sleepy or exhausted.5. Statutory Maintenance ChartsEvery mine must maintain and update the following records:Air-sampling and Analysis Record: For analyzing gas levels in underground mines.Stone Dust Barrier Record: To prevent coal dust explosions.Support Plan (SCAMP): A chart for monitoring strata control and roof bolting (Strata Control and Monitoring Plan).
